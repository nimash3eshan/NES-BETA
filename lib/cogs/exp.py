...
class Exp(Cog):
	...
	async def check_lvl_rewards(self, message, lvl):
		if lvl >= 50: # Red
			if (new_role := message.guild.get_role(828215413560836116)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(828215127593451570))

		elif 40 <= lvl < 50: # Yellow
			if (new_role := message.guild.get_role(828215127593451570)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(828214937842352138))

		elif 30 <= lvl < 40: # Green
			if (new_role := message.guild.get_role(828214937842352138)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(828214704794370050))

		elif 20 <= lvl < 30: # Blue
			if (new_role := message.guild.get_role(828214704794370050)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(828214267656142878))

		elif 10 <= lvl < 20: # Purple
			if (new_role := message.guild.get_role(828214267656142878)) not in message.author.roles:
				await message.author.add_roles(new_role)
				await message.author.remove_roles(message.guild.get_role(828213728225918996))

		elif 5 <= lvl < 9: # Black 
			if (new_role := message.guild.get_role(828213728225918996)) not in message.author.roles:
				await message.author.add_roles(new_role)
	...
	async def on_ready(self):
		if not self.bot.ready:
			self.levelup_channel = self.bot.get_channel(828216176249274388)
